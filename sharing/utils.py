...
def send_email(subject, to_email, item, context=None, html_template_path=None):
    """
    ...
    img_url = item.photo.url[1:] # remove leading /
    try:
    ...
    img.add_header('Content-Id', '<item_image>')  # angle brackets are important
    img.add_header("Content-Disposition", "inline", filename="item_image")
...
def get_target_users_email(target_users):
    emails = [target_user.email for target_user in target_users]
    return emails